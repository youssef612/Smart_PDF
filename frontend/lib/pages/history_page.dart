...
class _HistoryPageState extends State<HistoryPage>
  ...
  void _applyFilter() {
    ...
    setState(() {
      _filtered = _history.where((item) {
        ...
        final matchSearch =
            q.isEmpty || type.contains(q) || fileName.contains(q);
        final matchType =
  ...
  void _openItem(Map<String, dynamic> item) {
    ...
    final itemFileName =
        widget.fileName ?? item['file_name']?.toString() ?? 'ملف';
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) => SummaryPage(
            ...
            pageCount: int.tryParse(item['page_count']?.toString() ?? '0') ?? 0,
            fromPage: item['from_page'] != null
                ? int.tryParse(item['from_page'].toString())
                : null,
            toPage: item['to_page'] != null
                ? int.tryParse(item['to_page'].toString())
                : null,
            filesService:
                FilesService(), // ← الـ سطر ده يتقفل بيه الـ الـ Block عشان الإيرور يروح تماماً
          ),
        ),
      );
        ...
        background: Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: [_red.withOpacity(0.08), _red.withOpacity(0.15)],
            ),
            borderRadius: BorderRadius.circular(20),
          ),
  ...
  String _questionTypeLabel(String type) {
    switch (type) {
      case 'multiple_choice':
        return isArabic ? 'اختيار متعدد' : 'multiple';
      case 'true_false':
                      ...
                      child: _isLoading
                          ...
                          : RefreshIndicator(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) =>
                                    _buildHistoryCard(_filtered[i], i),
                              ),
                            ),
                    ),
                  ],
