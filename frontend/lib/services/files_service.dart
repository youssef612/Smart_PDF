...
Future<List<Map<String, dynamic>>> getHistory() async {
  ...
  // 👇👇👇 أضيف هنا الـ chat methods 👇👇👇

  // =========================
  // 💬 CHAT WITH FILE
  // =========================

  /// إرسال رسالة إلى ملف محدد
  Future<Map<String, dynamic>?> sendChatMessage({
    required String fileId,
    required String message,
  }) async {
    try {
      print('💬 Sending message to file: $fileId');
      ...
      print('⚠️ Chat response not successful: ${response.data}');
      return null;
    } catch (e) {
  ...
  /// الحصول على سجل المحادثات لملف معين
  Future<List<Map<String, dynamic>>?> getChatHistory(String fileId) async {
    try {
      print('📋 Getting chat history for file: $fileId');

      final response = await _apiService.dio.get('/files/$fileId/chat');
      ...
      print('⚠️ Chat history not found');
      return null;
    } catch (e) {
  ...
  Future<bool> deleteChatMessage({
    ...
    try {
      final response = await _apiService.dio.delete(
        '/files/$fileId/chat/$messageId',
      );

      if (response.data['success'] == true) {
        print('✅ Chat message deleted');
        return true;
      }
      return false;
    } catch (e) {
      print('❌ deleteChatMessage error: $e');
      return false;
    }
  }

  /// مسح سجل المحادثات
  Future<bool> clearChatHistory(String fileId) async {
    try {
      final response = await _apiService.dio.delete(
        '/files/$fileId/chat',
      );
